...
if __name__ == '__main__':
    ...
    Transactions_que = []
    for ID in UserID:
        _ , trx = MakeUserAccount(ID, Users)
        Transactions_que.append(trx)    
    
    # while(True):
    #     if len(Transactions_que) != 0 and len(Transactions_que) > 5:
    #         GameData, Transactions_que = Miner.mining(Transactions_que)
    #     else:
    #         break            
    
    u1, u2,u3,u4,u5 = UserID[0], UserID[1], UserID[2], UserID[3], UserID[4]
    user1, user2, user3, user4, user5 = Users[u1], Users[u2],  Users[u3], Users[u4], Users[u5],
    
    #Mining
    GameData, Transactions_que = Miner.mining(Transactions_que)
    
    u1_u2_PC, trx1, trx2, PCID = CreatePrivateChannel(Users, u1, u2, 1000, 1000, 1, 1, PrivateChannel_Network,UserID)
    Transactions_que.append(trx1)
    Transactions_que.append(trx2)
    PC[PCID] = u1_u2_PC

    u2_u3_PC, trx1, trx2, PCID = CreatePrivateChannel(Users, u2, u3, 1000, 1000, 1, 1, PrivateChannel_Network,UserID)
    Transactions_que.append(trx1)
    Transactions_que.append(trx2)
    PC[PCID] = u2_u3_PC
    
    u3_u4_PC, trx1, trx2, PCID = CreatePrivateChannel(Users, u3, u4, 1000, 1000, 1, 1, PrivateChannel_Network,UserID)
    Transactions_que.append(trx1)
    Transactions_que.append(trx2)
    PC[PCID] = u3_u4_PC
 
    # print('TRANSACTION')
    # u1_u2_PC.Transaction(u1, u2, 250, 'Equipment', 1)   

    # EmergencyClosing(Users, 253, 474, Transactions_que, PrivateChannel_Network,UserID)
    # ClosePC(Users, 253, PCID, Transactions_que, PrivateChannel_Network, UserID)
    # print('-----------------------')
    # print(u1_u2_PC.record[-2:-1])
    # print('-----------------------')
    # u1_u2_PC.Transaction(u1, u2, 250, 'Equipment', 1)
    ...
    a= Scenario1(Users, UserID, u1, u4, 100, 'Equipment', 1, Transactions_que, PrivateChannel_Network, PC)
